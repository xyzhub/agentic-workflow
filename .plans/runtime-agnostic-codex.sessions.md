...
plan-judge) folded into S3. No brief had started; every read below was
re-resolved for the merged shape.
...
`--search`, `--json`, `-o/--output-last-message`, `--output-schema <FILE>`,
`--ephemeral`, `--ignore-rules`, `--ignore-user-config`, `-p/--profile`, and
`exec resume`. `codex execpolicy check --rules <PATH> <COMMAND>…` exists
(`-r/--rules` repeatable). `codex debug prompt-input` renders the model-visible
prompt input as JSON.
...
name files S1 creates, and lint's template-reference check fails if markdown
names a template that does not exist yet. Run S1 → S2 → S3.
...
- **Do**: (no probes — the execpolicy facts above are settled; never run
  ...
  2. `plugins/agentic-workflow/templates/codex.rules` per memo §8 — Starlark
     `prefix_rule` entries with `decision = "forbidden"` and a justification that
     reads like the matching guardrail hook message, covering `git push` in every
     form (including `git -C <dir> push`), `git commit`, `git tag` and
     `git push --tags` / `--follow-tags`, `gh pr create`, `gh pr merge`, and the
     §14 paid-promotion/publish hosts. Every rule carries `match` / `not_match`
  ...
  3. `plugins/agentic-workflow/tools/run-codex.mjs` — zero deps, Node ≥ 18:
     ...
     - prompt assembly in the memo §6 order: role prompt body with frontmatter
       ...
       `--note`.
     - flags from the role's `tools:`: no `Write`/`Edit` → `-s read-only`;
       ...
       <thread-id>` uses `codex exec resume`. **Never** pass `--ignore-rules` —
       it would disable the project-layer guardrail file that is the whole parity
       mechanism.
  ...
  4. `tools/run-codex-test.mjs` (repo root, zero deps, exit 0 = pass, same
     header-comment style as `tools/hook-test.mjs`): a fake `codex` shim on a temp
     PATH that records argv and emits canned JSONL events plus a last message,
     a temp git repo for tree-derived assertions, and these groups — schema
     (parses; strictness invariant: every object's `required` covers its
     ...
     with a justification and at least one `match` + one `not_match`; plus
     binary-backed `codex execpolicy check --rules <file> <example>` verdicts for
     every enumerated example, printing `SKIP (no codex binary)` when `codex` is
     absent — never a silent skip); adapter (flag derivation for the three tool
     shapes, prompt block order, skill inlining, schema validation and the
     non-JSON path, `changed_paths`, `high_impact_touched`, all three exit codes,
     and `--ignore-rules` absent from EVERY recorded invocation including the
     `--resume` path).
...
- **Verify**: `node tools/run-codex-test.mjs` exits 0 with every named case
  ...
  plugins/agentic-workflow/templates/codex.rules git push origin main` is
  forbidden and the same check on `git status` is not (paste both verdicts into
  the handoff entry); `node plugins/agentic-workflow/tools/run-codex.mjs --help`
...
- **Read budget**: ~540 lines read, ~830 lines written — the heaviest brief, at
  ...
  1–2 + 4's schema/rules groups in one session, step 3 + 4's adapter group in the
  next; log it as a deviation rather than rushing the adapter. Suits: `backend`.
...
- **Do**:
  ...
  7. `commands/tune.md` — `argument-hint` gains the runtime forms; alias
     ...
     the fresh copy into `.claude/agents/<role>.md` sets
     `runtime:`/`model:`/`effort:`; banner becomes `TUNED (runtime: codex, model:
     <model>, effort: <effort>) — `; the no-arg table gains a **runtime** column;
  ...
  8. `commands/connect.md` — a `codex` MODE beside `server`: the dispatch line
     near the top, then a section with these steps, each verified before the next
     ...
     the target repo and verify with `codex execpolicy check --rules
     ...
     `~/.codex/config.toml` — never a silent user-config edit; if the owner
     declines, stop, write no §10 row, and report that the guardrails would be
     loaded-but-disabled;
     ...
     its distillate — one run proving binary, auth, schema, rules AND trust;
  ...
  9. `commands/doctor.md` — one probe group: advisory "Runtimes: claude only —
     ...
     carrying exactly one fix (`/agentic-workflow:connect codex`). A missing trust
     entry is RED, not yellow: the rules file is present but inert, which reads as
...
### S3 — routing, protocol, record, and the permanent plan-judge (issue #79)
...
- **Reads**: `gh issue view 79 --repo xyzhub/agentic-workflow` (the plan-judge
  ...
  `CHANGELOG.md` lines 1–40 (the `## [Unreleased]` block + the 1.50.1 entry as
  the format model — 991 total); `README.md` whole (59);
  `plugins/agentic-workflow/README.md` — grep first (`grep -n
  "tune\|connect\|templates/\|tools/" plugins/agentic-workflow/README.md`) then
  read only the two matching ranges, ~90 of 303; `evals/run.mjs` lines 96–178
  (the scenario loop, the `checks.mjs` contract, judging);
  `evals/scenarios/mission-plan/scenario.md` + `checks.mjs` + `rubric.md` (43
  total — the three-file scenario shape);
...
- **Do**:
  ...
  3. `templates/WORKFLOW.md` — §3 gains one table row: hooks fire only on Claude
     ...
     docs-reminder replaced by the distillate's `high_impact_touched`; §6 gains a
  ...
  5. Record: new eval scenario `evals/scenarios/codex-routing/` —
     `scenario.md` (frontmatter `budget-usd`, `pass-bar`, `judge-files`; the
     prompt drives `/agentic-workflow:mission "<name>" continue` over the
     fixture), `fixture/` (an adopted mini-project: `docs/WORKFLOW.md` §10 with a
     Runtimes row, a trio in `.plans/` whose S1 brief carries `runtime: codex`,
     and a fake `codex` shim on PATH so nothing reaches the API), `checks.mjs`
     (deterministic: the adapter was invoked with `--role`/`--brief`/`--out`, a
     distillate file was written, the ledger row and `Sessions used:` advanced,
     and the Agent tool was NOT used for that brief), `rubric.md` (weighted
     `- [w=N] id: criterion` lines). Then `CHANGELOG.md` — a `## [1.51.0] —
     <date>` entry moved out of `## [Unreleased]`, naming the plan-judge (#79) as
     well; the version bump in
...
- **Verify**: `node tools/lint.mjs` exits 0 — specifically the section check
  ...
  does); `node evals/run.mjs codex-routing` — tier 2, run ONLY with the owner's
  spend okay (~$1–5); without it, record in the ledger that the scenario is
  authored and deferred to the checkpoint, and say so in the report.
- **Read budget**: ~800 lines read, ~450 written — the second-heaviest brief,
  inside budget but with no slack. If it runs long, the split point is step 5
  (record + eval) versus steps 1–4 (routing + plan-judge); log it as a deviation.
  Suits: `devops`.

**Checkpoint ckpt-p1** ends the mission — ONE independent `reviewer` (fresh
context, one-shot) over the whole diff. **Fable required**: the diff carries the
...
claim), and that the CHANGELOG, the version and issue #79's scope all match. Then
