...
| `git commit` / turn end | **Beat-enforcer** (governance) — nudges a required-but-unchecked ledger beat (`chronicler` at close, `reviewer` at a checkpoint) at the overdue moment; suppressed when the beat isn't due (unfinished work or an unreleased blocker above it, or the row itself held); advisory, never blocks |
