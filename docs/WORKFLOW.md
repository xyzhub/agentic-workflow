...
| **Runtimes** | `claude` (default) · codex: `gpt-5.6-sol` (connected 2026-09-17; `gpt-6-astra` needs codex ≥ 0.154, installed 0.146.0) · rules: `.codex/rules/agentic-workflow.rules` · trust: user-layer entry present (`~/.codex/config.toml`, 2026-09-17) · shell wrapper: `/bin/zsh -lc '<cmd>'` (from the round-trip event stream; execpolicy matched `git commit` THROUGH the wrapper — rejected commands emit no command item, only the model's report quoting the rule justification) · spawn note: close stdin (`</dev/null`) or `codex exec` waits on the pipe |
