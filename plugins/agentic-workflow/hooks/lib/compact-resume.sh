...
#          judged against the transcript, never the clock). CURRENT only when
#          the handoff's mtime is newer than the transcript's — the last append
#          is the latest possible moment a budget-band crossing happened, so
#          `handoff -nt transcript` proves it postdates any crossing (the same
#          conservative proxy as handoff-budget.sh, disclosed at ckpt-p1).
#          Everything else — older handoff, missing or unreadable
...
#          current; fail closed toward suspect.
...
Then re-state the current brief's remaining Do/Verify items before continuing."
  ...
  if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ] && [ "$HANDOFF" -nt "$TRANSCRIPT" ]; then
    MSG="♻️ Context was just COMPACTED — what you hold now is a summary, not the record.
