...
| **Version pin** | _(the file and key this project bumps on release — e.g. `package.json` `version`, `pyproject.toml` `project.version`, a plugin manifest. `none` → the project is unversioned and the mission-close `version bumped + stamped` row fires immediately)_ |
