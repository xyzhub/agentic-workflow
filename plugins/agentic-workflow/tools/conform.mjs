...
const LADDER = [
  ...
  { id: 'profile-test-users-row', since: '1.48.2',
    check: () => has10('Test users') ? true : '§10 has no **Test users / auth access** row (docs/AUTH.md or `none` — without it every session re-discovers or asks for test credentials)',
    fix: 'add the §10 **Test users** row pointing at docs/AUTH.md (templates/auth.md; seeded credentials only) or `none`; /agentic-workflow:sync appends it' },
